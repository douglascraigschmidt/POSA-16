...
public class DownloadImagesBoundService 
     ...
     * request Messages sent from the client app.
     */
    private Messenger mRequestMessenger = null;

    /**
     * This class handles messages sent from a client app in a pool of
	 ...
	 * a client app.  The Message it receives contains the
	 * Messenger used to reply to the client app and the URL of
	 * the image to download.  This image is stored in a local
	 * file on the local device and image file's URI is sent back
	 * to the client app via the Messenger passed with the
	 * message.
	 */
	 ...
	 * the Activity via the @a replyMessenger.
	 */
	public void sendPath(Messenger replyMessenger, 
		...
		// Send the replyMessage back to the client app.
		// TODO -- you fill in here.
	    } catch (RemoteException e) {
		Log.e(getClass().getName(),
		      "Exception while sending reply message back to client app.",
		      e);
	    }
	}
